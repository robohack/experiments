 ...
 * compile-command: (let ((fn (file-name-sans-extension (file-name-nondirectory (buffer-file-name))))) (concat (default-value 'compile-command) " OPTIM='-O3' CPPFLAGS='-DUSE_VOLATILE' LDLIBS=tatomic-sigusr.c " fn " && echo 'send SIGUSR1 while running'"))
 * End:
 */
